...
public class TablePanel extends JPanel implements ActionListener, ChangeListener,
    ...
    private int seat, playerIndex, opponentIndex;
    private Player opponent, player;
    ...
    private void drawNames(Graphics2D g) {        
        ...
        g.setColor(Color.WHITE);
        try {
            int p0StringWidth = (int)fm.getStringBounds(opponent.getName(),g).getWidth();
            int p1StringWidth = (int)fm.getStringBounds(player.getName(),g).getWidth();                                               
            g.drawString(opponent.getName(), this.getWidth() / 2 - p0StringWidth / 2, NAME_Y);
            g.drawString(player.getName(), this.getWidth() / 2 - p1StringWidth / 2, this.getHeight() - NAME_Y);            
        } catch (NullPointerException ex) {
    ...
    private void drawStacks(Graphics2D g) {                  
        ...
        g.setFont(tableFont.deriveFont(SMALL_FONT_SIZE));
        FontMetrics fm = g.getFontMetrics();
        int p0StringWidth = fm.stringWidth("Chips: $" + gamestate.getStackSize(opponentIndex));
        int p1StringWidth = fm.stringWidth("Chips: $" + gamestate.getStackSize(playerIndex));
        g.drawString("Chips: $" + gamestate.getStackSize(opponentIndex), this.getWidth() - p0StringWidth - STACK_X, STACK_Y);
        g.drawString("Chips: $" + gamestate.getStackSize(playerIndex), this.getWidth() - p1StringWidth - STACK_X, this.getHeight() - STACK_Y);
    }
    ...
    private void drawScores(Graphics2D g) {        
        FontMetrics fm = g.getFontMetrics();
        int p0StringWidth = fm.stringWidth("Amount won: $" + opponent.getScore());
        int p1StringWidth = fm.stringWidth("Amount won: $" + player.getScore());
        g.setFont(tableFont.deriveFont(SMALL_FONT_SIZE));
        g.setColor((opponent.getScore() >= player.getScore() ? Color.GREEN : Color.RED));
        g.drawString("Amount won: $" + opponent.getScore(), this.getWidth() - p0StringWidth - SCORE_X, SCORE_Y);
        g.setColor((player.getScore() >= opponent.getScore() ? Color.GREEN : Color.RED));
        g.drawString("Amount won: $" + player.getScore(), this.getWidth() - p1StringWidth - SCORE_X, this.getHeight() - SCORE_Y);
    }
    ...
    private void drawGameOver(Graphics2D g) {
        ...
        Player winner = (player.getScore() > opponent.getScore() ? 
            player : opponent);        
     ...
     * @param seat a int for seat correction
     */
    public void update(Gamestate gamestate, List<Player> players, int seat) {
        this.gamestate = gamestate;
        this.players = players;
        playerIndex = gamestate.getPlayerPosition();
        opponentIndex = (gamestate.getPlayerPosition()+1) % gamestate.getNumPlayers();
        setSeat(seat);
        repaint();
    }
     ...
     * @param gameOver True for gameover
     */
     ...
     * @param hideStacks true for hiding stacksizes
     */
    ...
    /**
     * Set the winning hand string to display
     * @param winningHandString a String representing a human readable winning hand
     */
    ...
    /**
     * Set the players seat setup
     * @param seat an int representing the player's seat
     */
    public void setSeat(int seat) {
        this.seat = seat;
        int opponentSeat = (seat+1)%2;
        player = players.get(seat);
        opponent = players.get(opponentSeat);
    }
